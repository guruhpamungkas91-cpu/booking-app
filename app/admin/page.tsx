...
<div className={`backdrop-blur-2xl border rounded-2xl sm:rounded-3xl shadow-2xl bg-zinc-950/80 overflow-hidden ${themeStyles.borderAccent}`}>
  {loading ? (
    ...
    /* PERBAIKAN 1: Tambahkan pr-6 atau pr-8 pada wrapper tabel */
    <div className="w-full overflow-x-auto pr-6 sm:pr-8">
      {/* PERBAIKAN 2: Naikkan min-w-[1250px] menjadi min-w-[1450px] */}
      <table className="w-full text-left border-collapse min-w-[1450px]">
        <thead>
          <tr className="border-b border-zinc-800/80 bg-zinc-950/95 text-[10px] font-black uppercase tracking-widest text-zinc-400 select-none">
            ...
            {/* PERBAIKAN 3: Naikkan min-w kolom Status dari 150px ke 180px untuk menampung tombol Refund */}
            <th onClick={() => handleSort('status')} className={`p-3.5 sm:p-4.5 cursor-pointer transition hover:${themeStyles.textAccent} min-w-[180px]`}>
              <div className="flex items-center gap-1.5 whitespace-nowrap">
                <span>Status</span>
            ...
            {/* PERBAIKAN 4: Naikkan min-w kolom Aksi dari 100px ke 120px */}
            <th className="p-3.5 sm:p-4.5 text-center whitespace-nowrap pr-8 min-w-[120px]">Aksi</th>
          </tr>
        </thead>
          ...
          {filteredReservations.map((item) => {
            ...
            return (
              <tr key={item.id} className="hover:bg-zinc-800/30 transition-colors">
                ...
                {/* PERBAIKAN 5: Pasang min-w-[120px] di TD ini juga */}
                <td className="p-3.5 sm:p-4.5 text-center pr-8 min-w-[120px]">
                  <button
